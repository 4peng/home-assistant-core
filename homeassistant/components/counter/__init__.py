...
class Counter(collection.CollectionEntity, RestoreEntity):
    ...
    async def async_update_config(self, config: ConfigType) -> None:
        ...
        self.async_write_ha_state()
"""Re-engineered Counter Component."""
...
CONFIG_SCHEMA = vol.Schema(
    {DOMAIN: cv.schema_with_slug_keys(vol.All(_none_to_empty_dict, STORAGE_FIELDS))},
    extra=vol.ALLOW_EXTRA,
)
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    component.async_register_entity_service(SERVICE_RESET, None, "async_reset")
    component.async_register_entity_service(
        SERVICE_SET_VALUE, {vol.Required(VALUE): cv.positive_int}, "async_set_value"
    )
...
# ---------------------------
class Counter(collection.CollectionEntity, RestoreEntity):
    """Re-engineered Counter with centralized range computation."""
    ...
    def extra_state_attributes(self) -> dict:
        ...
        if self._config.get(CONF_MINIMUM) is not None:
            ret[CONF_MINIMUM] = self._config[CONF_MINIMUM]
        if self._config.get(CONF_MAXIMUM) is not None:
            ret[CONF_MAXIMUM] = self._config[CONF_MAXIMUM]
        return ret
    ...
    def compute_next_state(self, state: int | None) -> int | None:
        if state is None:
            return None
        if (minimum := self._config.get(CONF_MINIMUM)) is not None:
            state = max(minimum, state)
        if (maximum := self._config.get(CONF_MAXIMUM)) is not None:
            state = min(maximum, state)
        return state


    async def async_added_to_hass(self) -> None:
        await super().async_added_to_hass()
        if self._config.get(CONF_RESTORE) and (state := await self.async_get_last_state()):
            self._state = self.compute_next_state(int(state.state))

    @callback
    def async_increment(self) -> None:
        step = self._config[CONF_STEP]
        self._state = self.compute_next_state(self._state + step)
        self.async_write_ha_state()
    ...
    @callback
    def async_set_value(self, value: int) -> None:
        if self.compute_next_state(value) != value:
            raise ValueError(f"Value {value} out of allowed range")
        self._state = value
        self.async_write_ha_state()