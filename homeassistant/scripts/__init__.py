...
def run(args: list[str]) -> int:
    """Run a script."""
    scripts = _discover_scripts(os.path.dirname(__file__))
    ...
    if not _install_dependencies(script, config_dir):
        return 1
...
def _discover_scripts(path: str) -> list[str]:
...
def _install_dependencies(script: Any, config_dir: str) -> bool:
    """Install requirements for the script if necessary."""
